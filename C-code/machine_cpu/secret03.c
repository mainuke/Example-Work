 ...
 *
 * Tests trying to call disassemble() with the value of instr_num being
 * larger than the number of memory words, and also with the value of
 * data_num being larger than the number of memory words, which are both
 * invalid.
 *
...
int main() {
  ...
  assert(!disassemble(program, NUM_WORDS + 4, 4));
  assert(!disassemble(program, 4, NUM_WORDS + 4));
